...
void _lldpPortConfigTable_initialize_interface(
  ...
  DEBUGMSGTL(
      ("internal:lldpPortConfigTable:_lldpPortConfigTable_initialize_interface",
       "called\n"));
  ...
  reginfo = netsnmp_handler_registration_create(
      ...
      lldpPortConfigTable_oid_size, HANDLER_CAN_BABY_STEP | HANDLER_CAN_RONLY);
...
int lldpPortConfigTable_index_from_oid(netsnmp_index *oid_idx,
  ...
  DEBUGMSGTL(("verbose:lldpPortConfigTable:lldpPortConfigTable_index_from_oid",
              "called\n"));
...
lldpPortConfigTable_allocate_rowreq_ctx(void *user_init_ctx) {
  ...
  if (NULL == rowreq_ctx) {
    snmp_log(
        LOG_ERR,
        "Could not allocate memory for a lldpPortConfigTable_rowreq_ctx.\n");
    return NULL;
  }
...
static int _mfd_lldpPortConfigTable_pre_request(
  ...
  if (1 != netsnmp_row_merge_status_first(reginfo, agtreq_info)) {
    DEBUGMSGTL(
        ("internal:lldpPortConfigTable", "skipping additional pre_request\n"));
...
static int _mfd_lldpPortConfigTable_post_request(
  ...
  if (1 != netsnmp_row_merge_status_last(reginfo, agtreq_info)) {
    DEBUGMSGTL(
        ("internal:lldpPortConfigTable", "waiting for last post_request\n"));
    return SNMP_ERR_NOERROR;
  }
...
_lldpPortConfigTable_get_column(lldpPortConfigTable_rowreq_ctx *rowreq_ctx,
  ...
  case COLUMN_LLDPPORTCONFIGTLVSTXENABLE: {
    ...
    rc = lldpPortConfigTLVsTxEnable_get(rowreq_ctx, (u_long *)var->val.string);
    var->val_len = 0;
    while (0 != mask) {
      ++idx;
...
static int _cache_load(netsnmp_cache *cache, void *vmagic) {
  ...
  return lldpPortConfigTable_container_load((netsnmp_container *)cache->magic);
}
...
static void _container_item_free(lldpPortConfigTable_rowreq_ctx *rowreq_ctx,
                                 void *context) {
  DEBUGMSGTL(("internal:lldpPortConfigTable:_container_item_free", "called\n"));
...
static void _container_free(netsnmp_container *container) {
  ...
  CONTAINER_CLEAR(container, (netsnmp_container_obj_func *)_container_item_free,
                  NULL);
}
