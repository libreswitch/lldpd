...
class lldpTest (HalonTest):
    ...
    def bring_interface_up (self, switch_number, interface_number):
        info("\n")
        info("bringing interface %d up for switch %d\n" \
                % (interface_number, switch_number))
        s = self.switch_variable(switch_number)
        # set admin state & autoneg
        command = "ovs-vsctl set interface " + str(interface_number) + \
    ...
    def wait_for_lldp_packets_exchange (self):
        info("\n")
        info("waiting for some lldp packets to be exchanged\n")
        count = 0
        while (count < 30):
            time.sleep(1)
            info(".")
            ...
            if (crashed == True):
                info("\nlldp on switch 1 must have crashed; " + \
                     "old pid %s new pid %s\n" % (self.pid1, pid))
                assert 0, "lldp crashed on switch 1, exiting"

            crashed, pid = self.lldp_process_restarted(2)
            if (crashed == True):
                info("\nlldp on switch 2 must have crashed; " + \
                     "old pid %s new pid %s\n" % (self.pid2, pid))
                assert 0, "lldp crashed on switch 1, exiting"
            count = count + 1
    #
    ...
    def verify_lldp_rx_count_after_crash (self, switch_number, interface_number):
        info("\n")
        info("killing lldp on switch %d and verifying rx counts on interface %d\n" \
             % (switch_number, interface_number))
        s = self.switch_variable(switch_number)
        ...
        if (rxc_after_crash >= rxc_before_crash):
            ...
            info("switch %d interface %d test FAILED: before %d, after %d\n" \
                 % (switch_number, interface_number, \
                    rxc_before_crash, rxc_after_crash))
