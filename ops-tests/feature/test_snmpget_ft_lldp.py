...
def snmpget(ops1, version, accesscontrol, host, oid, extrav3conf=None):
    if version is "v1":
        retstruct = ops1("snmpget -v1 -c" + accesscontrol + ""
                         " " + host + " " + oid + ""
                         " ", shell='bash')
        return retstruct
    elif version is "v2c":
        retstruct = ops1("snmpget -v2c -c" + accesscontrol + ""
                         " " + host + " " + oid + ""
                         " ", shell='bash')
        return retstruct
    elif version is "v3":
        retstruct = ops1("snmpget -v3 " + extrav3conf + " "
                         "" + host + " " + oid + " ", shell='bash')
        return retstruct
...
def snmpget_v1_test_local(ops1):
    ...
    assert "INTEGER: 48" in retstruct, "snmpget "
...
def snmpget_v3_test_local(ops1):
    ...
    retstruct = snmpget(ops1, "v3", "None", "localhost", ""
                        ".1.0.8802.1.1.2.1.1.1.0",
                        "-u testv3user -l authNoPriv -a md5"
                        " -A password")
    assert "INTEGER: 48" in retstruct, "snmpget "
...
def snmpget_v1_test_remote(hs1):
    ...
    retstruct = hs1("snmpget -v1 -cpublic 10.10.10.4:161"
                    " .1.0.8802.1.1.2.1.1.1.0")
    assert "INTEGER: 48" in retstruct, "snmpget"
    " v1 failed to get configured value from the OVSDB"
...
def snmpget_v2c_test_remote(hs1):
    ...
    retstruct = hs1("snmpget -v2c -cpublic 10.10.10.4:161"
                    " .1.0.8802.1.1.2.1.1.1.0")
    assert "INTEGER: 48" in retstruct, "snmpget"
    " v2c failed to get configured value from the OVSDB"
